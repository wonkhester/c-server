...
void handle_root(int client_socket, HTTP_Request request) {
  ...
  if (html_content) {
    send_http_response(client_socket, 200, html_content, "text/html");
    free(html_content);
  } else {
...
void handle_yo(int client_socket, HTTP_Request request) {
  send_http_response(client_socket, 200, "This is the mega yo Page.", "text/html");
}

void handle_about(int client_socket, HTTP_Request request) {
  send_http_response(client_socket, 200, "This is the About Page.", "text/html");
}

void handle_submit(int client_socket, HTTP_Request request) {
  if (request.body != NULL) {

    ParsedURLData parsed_body = parse_url_encoded_body(request.body);
    ...
    send_http_response(client_socket, 200, response, "text/plain");
  } else {
    send_http_response(client_socket, 400, "No data received", "text/plain");
  }
}
...
void handle_update(int client_socket, HTTP_Request request) {
  if (request.body) {
    ...
    send_http_response(client_socket, 200, response_body, "text/plain");
  } else {
